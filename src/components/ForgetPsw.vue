...
export default {
    ...
    methods: {
        verifyEmailHandler: function (formName) {
            this.$refs[formName].validate((valid) => {
                if (valid) {
                    ...
                    this.axios.get('http://47.102.116.29/api/Users/Verifycode?id=' + this.emailForm.email + '&option=2', {
                            params: {
                                userAccount: this.emailForm.email
                            }
                        })
                        .then((response) => {
        ...
        finishHandler: function (formName) {
            this.$refs[formName].validate((valid) => {
                if (valid) {
                    this.axios.post('http://47.102.116.29/api/Users/ChangePassword', {
